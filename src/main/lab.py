...
"""
We will create an agent that will be able to deduce the length of a word or the cube of a number.
...
llm = AzureChatOpenAI(deployment_name=deployment, model_name="gpt-3.5-turbo")

"""
The two functions below will serve as tools for the agent. 
In other words, they are processes that the agent will be able to choose from to complete a given task.
...
def get_cube_of_number(number) -> int:
    # TODO: write a description of what this tool does
    """Returns the cube of a number. In other words, returns the number to the power of 3."""
...
Finally, by setting verbose=True, we can see what the agent is thinking/doing in the console. 
"""
